...
async function readTags(skip: number = 0, limit: number = 25): Promise<TagPublicRow[]> {
	const token = localStorage.getItem("access_token") || "";
	const response = await fetch(`${OpenAPI.BASE}/api/v1/tags/?skip=${skip}&limit=${limit}`, {
		headers: {
...
async function updateTag(tagId: string, name: string): Promise<TagPublicRow> {
	...
	if (!response.ok) throw new Error((await response.text()) || "Failed to update tag");
	return response.json();
}
			...
			<Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
				<DialogContent className="sm:max-w-md">
					...
					<div className="grid gap-2 py-2">
						...
						<Input id="edit-tag-name" value={editName} onChange={(e) => setEditName(e.target.value)} />
					</div>
					<DialogFooter>
						<DialogClose asChild>
						...
						<LoadingButton loading={editMutation.isPending} onClick={() => editMutation.mutate()} disabled={!editName.trim()}>
							Save
						</LoadingButton>
					</DialogFooter>
				</DialogContent>
			...
			<Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>This tag will be permanently deleted. Are you sure?</DialogDescription>
					</DialogHeader>
		...
		<div className="flex flex-col gap-6">
			<div className="flex items-center justify-between gap-4">
				<div>
					...
					<p className="text-muted-foreground">Display and create tags from backend model.</p>
				</div>
				<Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
					...
					<DialogContent>
						...
						<div className="grid gap-2 py-2">
							...
							<Input id="tag-name" value={tagName} onChange={(e) => setTagName(e.target.value)} placeholder="e.g. skincare" />
						</div>
						<DialogFooter>
							<Button variant="outline" onClick={() => setIsCreateOpen(false)} disabled={createMutation.isPending}>
								Cancel
							</Button>
							<Button onClick={() => createMutation.mutate()} disabled={!tagName.trim() || createMutation.isPending}>
			...
			{isLoading ? <p className="text-muted-foreground">Loading...</p> : <DataTable columns={columns} data={tags} showPagination totalCount={Math.max(tags.length, (currentPage + 1) * pageSize)} currentPage={currentPage} pageSize={pageSize} onPageChange={setCurrentPage} />}
		</div>
	);
}
